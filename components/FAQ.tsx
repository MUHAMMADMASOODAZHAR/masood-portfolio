...
export default function FAQ() {
  ...
  const faqs: FAQItem[] = [
    ...
    {
      question: "Will I own the workflows, funnels, and integrations?",
    ...
    {
      question: "What information is required before starting?",
  ...
  return (
    <section id="faq" className="py-10 md:py-14 lg:py-16 bg-slate-50/50 dark:bg-slate-900/40 transition-colors duration-300">
        ...
        <div className="space-y-3.5">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="border border-slate-200/80 dark:border-slate-800 rounded-2xl overflow-hidden bg-white dark:bg-slate-900 transition-colors duration-300 shadow-xs"
              >
                <button
                  ...
                  id={`faq-btn-${idx}`}
                  className="w-full flex items-center justify-between p-5 md:p-6 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 hover:bg-slate-50 dark:hover:bg-slate-850 transition-colors duration-200 cursor-pointer"
                >
                  <div className="flex items-center gap-3.5 pr-4">
                    <span className="text-xs font-extrabold text-blue-600 dark:text-blue-400 font-mono shrink-0">
                      {stepNum}.
                    </span>
                    <span className="font-bold text-slate-950 dark:text-white text-base md:text-[17px] leading-snug">
                      {faq.question}
                    </span>
                  </div>

                  <div className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 border border-slate-200/60 dark:border-slate-700 text-slate-600 dark:text-slate-300">
                    {isOpen ? <Minus className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" /> : <Plus className="w-3.5 h-3.5" />}
                  </div>
                </button>
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="p-5 md:p-6 pt-0 border-t border-slate-100 dark:border-slate-800 text-base text-slate-650 dark:text-slate-350 leading-relaxed font-normal">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
